...
const GameShellV2Inner = ({ children, hideNavigation: forceHideNavigation, showNavigation: forceShowNavigation, hideLogo, defaultRailMinimized = false, defaultSectionsPanelClosed = false, spaceOverride }: GameShellV2Props) => {
    ...
    const getSpaceFromPath = (pathname: string): string | undefined => {
        ...
        // Day 111 (Sasha 2026-07-01): Founder Cockpit is the public
        // front-door for Founder Life's Work Navigation, but it belongs
        // conceptually inside BUILD. Keep the BUILD rail/pane active
        // while rendering the shareable /build/cockpit route and its dashboard.
        if (pathname === "/build/cockpit" || pathname.startsWith("/build/cockpit/")) return "build";
    ...
    const loadProfile = async (userId: string) => {
        try {
            ...
            if (row?.last_zog_snapshot_id) {
                const { data: snapshotData } = await supabase
                    .from("zog_snapshots")
                    .select("excalibur_data, archetype_title")
                    .eq("id", row.last_zog_snapshot_id)
                    .maybeSingle();
        ...
        <div className={cn(isAiOsRoute ? "h-dvh overflow-hidden" : "min-h-dvh", "bg-[#0a0a1a]")}>
            ...
            <div className={cn("relative z-10 hidden lg:flex isolate", isAiOsRoute ? "h-dvh min-h-0 overflow-hidden" : "min-h-dvh")}>
                ...
                <SpacesRail
                    ...
                    isAuthenticated={authChecked ? !!user : undefined}
