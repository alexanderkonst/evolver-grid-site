...
export const DailyLoopLayout = ({
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title={t('dailyLoopLayout.me.title')}
        icon={<Sparkles className="w-5 h-5 text-[#8460ea]" aria-hidden />}
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-[#2c3150]/60">{t('dailyLoopLayout.me.welcomeBack')}</p>
            <p className="text-xl font-bold text-[#2c3150]">
              {profileName ? `${profileName}` : t('dailyLoopLayout.me.playerFallback')}
            </p>
            {archetypeTitle && (
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-[#2c3150]">
            {typeof level === "number" && typeof xpTotal === "number" && (
              <span className="inline-flex items-center gap-2 rounded-full bg-[#a4a3d0]/20 px-3 py-1 font-semibold">
                ...
                {t('dailyLoopLayout.me.levelXp', { level, xp: xpTotal })}
              </span>
            )}
            {streakDays && streakDays > 0 && (
              <span className="inline-flex items-center gap-2 rounded-full bg-[#cea4ae]/20 px-3 py-1 text-[#2c3150]">
                ...
                {t('dailyLoopLayout.me.streak', { count: streakDays })}
              </span>
            )}
          </div>
        </div>
      ...
      <SectionCard
        title={t('dailyLoopLayout.myLife.title')}
        ...
        {lowestDomains.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {lowestDomains.map((domain) => (
              ...
              >
                {t('dailyLoopLayout.myLife.domainFocus', { domain })}
              </span>
            ))}
          </div>
        ) : (
          <p className="text-sm text-[#2c3150]/70">{t('dailyLoopLayout.myLife.empty')}</p>
        )}
      </SectionCard>

      <SectionCard
        title={t('dailyLoopLayout.nextMove.title')}
          ...
          <div className="rounded-2xl border border-[#b1c9b6]/30 bg-[#b1c9b6]/10 p-4 sm:p-5">
            {isLoadingAction ? (
              <div className="flex items-center gap-3 text-[#2c3150]">
                ...
                <p className="font-semibold">{t('dailyLoopLayout.nextMove.loading')}</p>
              </div>
            ) : actionError ? (
              <div className="space-y-3">
                <p className="text-lg font-semibold text-[#2c3150]">{t('dailyLoopLayout.nextMove.errorTitle')}</p>
                <p className="text-sm text-[#2c3150]/70">{actionError}</p>
                {onRetryAction && (
                  <Button variant="secondary" size="sm" onClick={onRetryAction}>
                    {t('dailyLoopLayout.nextMove.tryAgain')}
                  </Button>
                )}
              </div>
            ) : recommendedAction ? (
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-[#2c3150]">{recommendedAction.tag || t('dailyLoopLayout.nextMove.actionTagFallback')}</span>
                ...
                {recommendedAction.rationale && (
                  <p className="text-sm text-[#2c3150]/60">{t('dailyLoopLayout.nextMove.whyPrefix')} {recommendedAction.rationale}</p>
                )}
                {recommendedAction.alternates && recommendedAction.alternates.length > 0 && (
                  <div className="pt-2">
                    <p className="text-xs font-semibold text-[#2c3150] uppercase ">{t('dailyLoopLayout.nextMove.alternatives')}</p>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-2">
                  ...
                  >
                    <BoldText>{t('dailyLoopLayout.nextMove.doItNow')}</BoldText>
                    <ArrowRight className="h-4 w-4" aria-hidden="true" />
                  </Button>
                  <Button asChild variant="secondary" size="sm">
                    <Link to={freedomModeUrl} onClick={onFreedomMode}>{t('dailyLoopLayout.nextMove.freedomMode')}</Link>
                  </Button>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-lg font-semibold text-[#2c3150]">{t('dailyLoopLayout.nextMove.emptyTitle')}</p>
                <p className="text-sm text-[#2c3150]/70">{t('dailyLoopLayout.nextMove.emptyBody')}</p>
                <Button asChild variant="secondary" size="sm">
                  <Link to={freedomModeUrl}>{t('dailyLoopLayout.nextMove.openFreedomMode')}</Link>
                </Button>
              </div>
            )}
          </div>
