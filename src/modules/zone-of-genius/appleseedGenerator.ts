...
- Life Scene: A founder leans across a wooden table, eyes a little wild, mid-rant about three half-built systems. You let it land for a beat, then say one sentence. Their shoulders drop. They write it down, look up, ask, "Can we do this Tuesday?"
  ...
  Top Talent Profile (deep block — note the register: SECOND-PERSON addressing the reader directly with "you / your". Plain everyday language. No insider jargon.):
    ...
    complementaryPartner.synergy: "Your best partner is a grounded closer-operator whose craft is finishing: running sales calls, setting prices, sending proposals, and installing the weekly cadence that turns plans into revenue. Their genius is sensing what a real buyer will say yes to today, then making the smallest set of moves that gets the yes. Their being-pattern is steady, pragmatic, and slightly impatient with endless refining. You are both aimed at the same thing, that the medicine reaches the person who needs it, and you stand at different points on the way there. What appears between you is a business that is both true and sellable: the real signal named and built into a path, and that path turned into booked calls, signed agreements, and releases that ship on a date."  // "Their" here = the partner, not the subject — that's correct second-person framing
    monetizationAvenues:
...
Return a JSON object with this exact structure:
{
  ...
  "complementaryPartner": {
    "synergy": "string — ONE tight prose paragraph (3-5 sentences) describing the partner whose position most productively meets this one. FRAMING LAW: this partner is a whole practitioner in their own right, not a set of missing pieces. Never describe them as filling a gap, covering a weakness, completing the subject, or supplying what the subject lacks. The structure is: same center, different position. Both are aimed at the same thing and stand at different points on the way there. Fuse three layers into continuous flow, not labeled sections: what they do (their own craft, stated as mastery, never as the subject's deficiency), how they sense or create (their irreducible signature), and the being-pattern they carry. Then close with what appears BETWEEN the two of them: the third thing that shows up when two complete positions align, which belongs to neither alone. Read like a paragraph in a profile, not a four-row table. No bullet lists. No headings. No 'Skills-wise…, Genius-wise…' labels. One continuous read. SECOND-PERSON addressing the subject (\"your best partner is…\"). DUAL-ACTOR DISCIPLINE: the PARTNER stays \"they / them / their\" throughout — never collapse the partner into \"you\". (Bug example to avoid: \"Your best partner loves finishing what YOU started\" — that second \"you\" is fine if it really is the subject's work; but \"a partner who senses what YOU will say yes to\" when meaning the partner senses what BUYERS will say yes to is wrong → \"what THEY will say yes to\".)",
...
export interface UnifyingRoleProfile {
   ...
   * unifying role on first contact, not because it supplies something
   * they lack but because it names precisely what they have been
   * standing in without words. */
