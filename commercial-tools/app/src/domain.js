...
export function learningRows(people, groupBy = 'template') { const map = new Map(); for (const p of people) { if (!p.outreach) continue; const key = groupBy === 'icp' ? p.outreach.icpId : groupBy === 'term' ? p.outreach.searchTerm || 'unknown' : groupBy === 'archetype' ? (p.archetype?.id || 'unlabelled') : p.outreach.templateId; const row = map.get(key) || { key, sent: 0, replied: 0, positive: 0 }; row.sent++; if (['replied', 'positive'].includes(p.outcome)) row.replied++; if (p.outcome === 'positive') row.positive++; map.set(key, row); } return [...map.values()].map(r => ({ ...r, replyRate: r.sent ? r.replied / r.sent : 0, positiveRate: r.sent ? r.positive / r.sent : 0 })).sort((a, b) => b.positiveRate - a.positiveRate || b.replyRate - a.replyRate); }
