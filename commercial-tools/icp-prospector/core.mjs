...
/* ── Term-learning helpers (mirrored verbatim in artifact.html) ─────────── */
export const MODS = ['next chapter','in transition','career pivot','exploring next move','independent advisor','portfolio','reinventing','building something new','on sabbatical','partnerships'];

export function termQuality(termStats, term){
  const s = termStats[term] || {}, d = s.downstream || {};
  return (d.paid||0)*1000 + (d.calls||0)*200 + (d.replied||0)*60 + (d.cashUsd||0)*0.5 + (s.highQuality||0)*8 + (s.found||0)*1;
}

export function freshCandidates(icp, used, current){
  const out = [];
  for (const k of icp.strong) for (const m of MODS){
    for (const t of [k+' '+m, m+' '+k]){
      const n = normalize(t);
      if (!used.has(n) && !current.has(n) && !out.some(x=>normalize(x)===n)) out.push(t);
    }
  }
  return out;
}
