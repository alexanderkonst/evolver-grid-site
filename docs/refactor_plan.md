...
- Phase 0: unified action shape and legacy mapping shipped; audit snapshot captured. DRI/ETA and rollback trigger now in `docs/roadmap.md`; XP router alignment still needs verification (confirm `calculateQuestXp` fields and `game_profiles` growth path totals match unified action payload).
...
**Phase 2 kickoff notes:** Added legacy action aggregation helper + fixtures, and introduced `completeAction` with practice wiring. Remaining: full source coverage, rec strategy across loops, quest/upgrade completion wiring, and tests.
