...
| `lib/upgradeSystem.ts` | transformation | `"upgrade"` | `upgrade:${id}` | `title`/`description` | required; fallback `"genius"` | none | use `timeEstimate` buckets | `sourceId` = upgrade id; `xp` from upgrade | prerequisites from upgrade dependencies |
