...
- [x] **Apr 25 Day 51 night: Monetization Strategy doc v1.0 created** — `docs/02-strategy/monetization_strategy.md` (NEW) is now the canonical reference for the dynamic, sequenced business-monetization strategy. Captures the formula (PolyForm NC + 10% rev-share + forkable kit + opt-in commons = Holonic Franchise), all six plays through Heart/Mind/Gut (Conscious Entrepreneur · Purpose Coach · Platform Distribution · Venture Studio · Community White-Label · System License), tensions/combinations/non-overlap test, four-phase sequenced rollout with exit criteria, implementation status table per play, 16 open decisions tracked. Cross-references established with holonic_franchise_model.md (parent), playbook P15 (brief inline summary now points here), alexanders_unique_business.md (Day 51 night entry mentions it), and this roadmap. **This is now the single source of truth** for how the venture earns money across all six commercial layers.
