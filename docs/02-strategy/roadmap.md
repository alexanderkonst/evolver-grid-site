...
| 47 | **Artifact seeding status UI** | UBB / Platform | 🟡 | Lovable's suggestion. Surface per-founder seeding state (which artifacts seeded / which stuck / which pending) somewhere visible — e.g. an admin row inside `/founders/:slug` or a banner on `/ubb` Canvas. Otherwise a partial-migration leaves Sasha guessing what landed. Scope-tight: read-only status, no re-trigger UI |
| 48 | **Security linter fixes (pre-existing warnings)** | Infra | 🟡 | Surfaced incidentally during Day-53 migration — Lovable noted *"Linter warnings are pre-existing and unrelated"* on the schema-constraint migration. Capture a separate pass to triage + close. File scope unspecified by Lovable; need to re-run linter to enumerate |
| 49 | **Publish after DB change (deploy schema + seed-founder-docs)** | Infra | 🟡 | The `user_business_artifacts_step_number_check` constraint loosening + the seed-founder-docs token-budget bump need to ride out via standard Lovable publish so prod state matches the migration applied during the session. Confirm both went live |
| 50 | **Verify module CMS wiring** | UBB / Platform | 🟡 | Lovable open todo. Probably checking that the seeded artifacts render correctly through the existing `/ubb/<slug>` artifact views (Generic + Compound). Validate by clicking through Alexander's now-seeded 16 artifacts on /ubb and confirming each card shows v1 locked + specificity 9.5 + content from the source `.md` |
---
