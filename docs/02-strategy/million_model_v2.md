# The $1M Model — v2.0

> *Day 170, August 29, 2026. Supersedes the v1 arithmetic in `million_pathway_brief.md`, which assumed 100 sessions a year and no delegation. Built on the containers in [`sovereign_founder_collective.md`](./sovereign_founder_collective.md) and the funnel rates in [`unique_business_playbook.md`](../03-playbooks/unique_business_playbook.md) § Funnel Conversion Math.*

---

## Executive summary

**Revenue = attributing founders × their revenue × 10%.** At the $1M mark that reads: ~100 attributing founders, ~$10M of founder revenue under the model.

*$1M is an arbitrary mark on the ruler, useful only because it makes the mechanism legible. The model is the mechanism and its rates; the number is a readout.*

Five findings from running the arithmetic:

1. **Reaching 100 attributing founders takes ~300 sessions, not 100.** At a 33% attribution rate, the session count and the outcome count are separated by a ratio of three.
2. **The share is collected from the founder's first dollar, not from $55,556.** The PMF container is 10% *of* the first $55,555, capped at $5,555. Cash therefore begins roughly 3-6 months after a session rather than 12-18. This pulls the $1M run-rate to **month 14-18**, not 20-26.
3. **Session capacity is no longer the binding constraint. Demand is.** At 10 sessions a week the constraint moves entirely to the top of the funnel.
4. **Demand runs at ~19% of capacity.** Baseline cold LinkedIn supplies 7 sessions a month against a 37/month delivery capacity. The constraint is not the calendar, the method or the price. It is the top of the funnel, and the reply rate is the one lever inside it.
5. **Volume sets the clock; attribution and founder revenue set the ceiling.** Doubling sessions halves time-to-$1M. It does not raise the $1M. Only the attribution rate and average founder revenue do that.


---

## Exhibit 1 — The ladder (v2)

| # | Container | Price | Format | Delegable |
|---|---|---|---|---|
| 0 | **Uniqueness session** | $250 | 1:1, short | **Yes, early.** Train and outsource |
| 1 | **Productize Yourself** | $555 | 1:1, ~2h | Later, via the recorded teaching block |
| 1b | **8-week container** | $2,000 1:1 · **$1,111 group** | PYS sits near the end | Rare practitioners, later |
| 2 | **Product Market Fit** | 10% of first $55,555, cap **$5,555** | ~6-week group cohort | Trainers |
| 3 | **Scale** | ongoing **10%** | assembled team | Yes |
| 4 | **Node** | 10% of the node's 10% | n/a | n/a |

The founder keeps 100%. No equity, no fund. The only from-pocket payments are rungs 0, 1 and 1b.

**Note on the group container:** Sasha has run group containers repeatedly, including the current one. The $1,111 group form of the 8-week container is not untested delivery; it is a known format at a new price point with normal expected learnings.

---

## Exhibit 2 — Capacity, and where the constraint actually sits

| Input | Base | Stretch |
|---|---|---|
| Sasha's sessions/week | 10 | 20 |
| Working weeks/year | 44 | 44 |
| **Sessions/year** | **440** | **880** |

Cohorts are group containers and never bind. Uniqueness sessions delegate immediately. The recorded teaching block removes ~40 minutes from every Productize Yourself session, and trainers eventually remove the session itself.

**Conclusion: the model does not saturate its originator.** At 440 sessions a year the binding constraint is no longer his calendar. It is whether 440 ripe founders can be found and reached.

---

## Exhibit 3 — The path to $1M

Assumptions, all stated: attribution rate **33%**; the 10% share begins at the founder's **first dollar**; a founder reaches early product-market fit in ~8 weeks and begins earning within **3-6 months**; average founder revenue reaching **$100k/yr** by their second year; ramp to full capacity over ~6 months.

| Year | Sessions | Cumulative attributing founders | Session + container fees | 10% share | **Total** |
|---|---|---|---|---|---|
| 1 | ~260 | ~85 | ~$220k | ~$110k | **~$330k** |
| 2 | ~440 | ~230 | ~$420k | ~$780k | **~$1.2M** |
| 3 | ~440 + nodes | ~380 | ~$470k | ~$1.9M | **~$2.4M+** |

**$1M annual run-rate crosses during year 2, around month 14-18.** Nodes are upside on this, not a requirement.

The composition inverts as it grows. Year 1 is ~67% session fees. Year 3 is ~80% share. **The business converts from a services practice into a royalty on founder success, and the conversion is automatic if the attribution rate holds.**

*This is a linear model of a system with compounding inputs (referrals from attributing founders, node formation, host referrals to other hosts). It is deliberately the floor, not the forecast.*

---

## Exhibit 3b — The practitioner cash question, and the law that resolves it

The problem: delivery increasingly involves other practitioners (integration work, trainers, the Scale team) whose costs land **before** the founder's revenue does. Success-coupled pricing has a working-capital shape.

Two facts remove most of it:

- The 10% starts at the founder's first dollar, so cash begins in months 3-6, not at $55,555.
- The $250 uniqueness session and the $1,111 / $2,000 container are from-pocket and immediate.

The remaining gap resolves with one rule, applied recursively:

> **Every container carries a small from-pocket price that pays its deliverer today, plus a share of the founder's 10% that pays them for the outcome.**

The same shape as the node arrangement, one layer down. Consequences: no separate practitioner budget is needed, nobody is paid before the founder earns anything, and every practitioner in the chain inherits the selection incentive, because they are paid only if the founder succeeds and attributes. The integration container currently has neither a price nor a share and needs both.

**Status: proposed, not decided.**

---

## Exhibit 4 — The demand engine

Sequence: **connection request first, message only after acceptance.** Every target hand-selected. That makes the reply rate a warm-message rate, not a cold-InMail rate, and it is the number the whole funnel turns on.

| Step | Working rate | Range |
|---|---|---|
| Connection request accepted | 50% | 40-60% |
| Accepted → reply with buying intent | **20%** | 15-25% |
| Reply → quiz → Direction Call | 33% | 30-40% |
| Direction Call → paid session | 50% | 40-50% |

**Sensitivity to the reply rate alone**, at LinkedIn's 20 requests a day (600/month):

| Reply rate | Requests per session | Sessions/month |
|---|---|---|
| 5% | 242 | 2.5 |
| 15% | 81 | 7.4 |
| **20%** | **61** | **9.9** |
| 25% | 48 | 12.4 |

### The three cases

| Case | Reply rate | Sessions/mo | Sessions/yr | Attributing founders/yr | Years to 100 |
|---|---|---|---|---|---|
| **Conservative** | 5% | **2.5** | 30 | 10 | 10.1 |
| **Baseline** | 15% | **7** | 84 | 28 | **3.6** |
| **Optimistic** | 25% | **12** | 144 | 48 | 2.1 |

**The finding this exposes: at every case, demand runs far below capacity.** Sasha can deliver 440 sessions a year. Baseline LinkedIn supplies 84, which is **19% of capacity.** The constraint is not his calendar, his method, or his pricing. It is entirely the top of the funnel.

Consequences, stated plainly:

- **Cold LinkedIn alone reaches $1M in roughly four years at baseline**, and never at the conservative case. It is a real engine and not a sufficient one.
- **The reply rate is worth more than any other single improvement at this stage.** Moving 15% to 25% cuts 1.5 years off the timeline and costs nothing but targeting and copy.
- Every additional channel is pure compression. There are 356 sessions a year of unused capacity waiting for them.

Three sources sit outside this arithmetic and are not counted: the existing network, which can be messaged without spending a request and is a one-time reservoir; inbound from posts and profile views; and warm intros. Boardy currently produces peers rather than buyers (six intros, zero clients), which is a brief problem.

**The later tier.** Community hosts change the volume ceiling substantially, since one host with ~30 members can supply a full cohort. That belongs after founder-level proof, not before it. Do not build the host offer until founders have been served and the model has run at the individual layer.

---

## Exhibit 5 — Sensitivity

Effect on year-3 revenue of moving one input alone:

| Input | Base | Move to | Effect |
|---|---|---|---|
| **Attribution rate** | 33% | 50% | **+52%** |
| **Avg founder revenue** | $100k | $150k | **+50%** |
| Sessions/year | 440 | 880 | +45%, and halves time-to-$1M |
| Crossing lag | 15mo | 9mo | Pulls $1M forward ~6 months |
| Nodes | 0 | 10 | +10-15%, growing |

**Attribution is the highest-leverage number in the business** and it is set at intake, by selection. See Technology 140.

---

## Exhibit 6 — Risks

| Risk | Severity | Mitigation |
|---|---|---|
| **Attribution decay.** Founders cross and do not attribute it, or exit the arrangement | **Critical** | Selection, not enforcement. Ripeness read at intake. See Technology 140 |
| Reply rate lands at 5% not 15% | **Critical** | Conservative case never reaches 100 attributing founders. Measure it over the first 200 requests before anything else is decided |
| **Practitioner cash gap** | High | Deliverers are paid before founders earn. See Exhibit 3b |
| Group container at a new price point | Low | Group delivery is well-practised. The $1,111 price and the 8-week shape carry normal expected learnings |
| Delegation dilutes quality | Medium | Uniqueness sessions delegate cleanly; PYS is the risk. Record the block first |
| $100k average founder revenue is a guess | Medium | Test against the nine existing canvases |

---

## Open questions

The category name. Node license terms. The purpose-entity threshold (~25 nodes, elastic). Who staffs the Scale tier and how they are paid. Whether the $1,111 group container holds.

---

## What the model says to do next

1. **Fire the LinkedIn system and log the reply rate over the first 200 requests.** The whole model swings between 2.5 and 12 sessions a month on that one number, and it is currently an estimate. Prove the model at the individual layer before touching the host channel.
2. **Record the teaching block.** It is the precondition for delegating anything.
3. **Rewrite the Boardy brief for buyer intent.** Six intros produced six good conversations and zero clients; the brief is currently producing peers.
4. **Price the integration container** and attach its share, per Exhibit 3b.
5. **Test the attribution rate against the nine existing canvases** so 33% stops being an estimate.

---

## Exhibit 5 — Assumptions vs Actuals (the live ledger)

> Added Day 177, September 7, 2026. Exhibit 4 states the working rates the whole forecast turns on. This exhibit is where those assumptions meet real numbers, append-only. **The rule that keeps it honest: a single data point never moves the line. An assumption moves only when the sample is large enough to trust and the actual sits outside the stated range.** The headline forecast recomputes only from a *moved* assumption, never from one encounter.

### The scoreboard

| Funnel step | Assumption (working · range) | Actual | n | Read | Moved? |
|---|---|---|---|---|---|
| Connection request accepted | 50% · 40-60% | **42%** | 12 | within range, under the working point (reconciled via get-sent-invitations) | No — inside range, n small |
| Accepted → reply (buying intent) | 20% · 15-25% | **1 reply** | 4 sent | first reply ~2h (Danil, positive); n far too small, and sends are not uniform (mirror-delivered vs deferred) | No |
| Reply → quiz → Direction Call | 33% · 30-40% | — | 0 | pending | — |
| Direction Call → paid session | 50% · 40-50% | — | 0 | pending | — |

**Forecast status: UNCHANGED.** Two honest reasons:

1. The one number we have (accept **45%**) is *within* the assumed band and slightly *under* the 50% working point — not above it. It clears the **35% ops floor** (so volume holds), but the 35% floor is a safety guard, not the forecast's assumption. Against the forecast's own number (50%), 45% is a small miss, not a beat.
2. The forecast swings almost entirely on the **reply rate** (Exhibit 4 sensitivity: 5% → 25% moves sessions/month from 2.5 to 12.4). That number has **zero data** — the first messages went out today. Moving the $1M timeline on an accept rate that isn't even a beat would be inventing a win.

### Change-log (append-only)

**2026-09-07 (Day 177) — first actuals logged, forecast held.**
- **Accept rate: 45% (5 of 11), Wave 1.** Above the 35% floor, below the 50% working assumption, inside the 40-60% range. Assumption unchanged; n=11 is too small to move a rate.
- **Reply rate: still unmeasured.** First three warm messages sent today (14:24 Danil, 14:58 Javier, 15:58 Emiliano). The clock on the load-bearing number starts now.
- **Action: none on the forecast.** Ledger opened. Next review when accept n ≥ 30, or the first 5 replies land, whichever comes first.

**2026-09-07/08 (Day 177-178) — first reply landed; forecast still held.**
- **Danil replied ~2 hours after first contact, positive and engaged.** He confirmed the mirror ("оно действительно плохо скейлится 😁"), asked how the method works, and asked for the story. First reply-with-buying-intent of the wave, and it came from the ripest, most-complementary target (the A/B hybrid).
- **Reply rate now has its first data point: 1 of 4 first-messages sent** (Danil, Javier, Emiliano, Juan). Three are under two days old and the sends are **not a clean sample** — message structure varies per person by design (mirror-delivered for Danil, mirror-deferred/consent-first for Juan), so reply rate must be read per message-type, not pooled. No computation yet; n=4.
- **Accepts: four of five now identified** (Danil, Emiliano, Javier, Juan). If Juan's acceptance postdates the firm "5" count, accepts = 6 and accept rate = ~55% (6/11); pending Sasha's confirmation. Accept assumption still unmoved either way (both inside the 40-60% band).
- **Action: forecast held.** The reply-rate clock is now genuinely running. The first signal is encouraging — fast, positive, from the best target — but n=4 with a confounded sample is nowhere near a move. Logged as signal, not as a forecast change.

**2026-09-09 — full connector reconcile of the denominator.** `get-sent-invitations` shows the settled campaign is **12 sends**, not 11 (added Cesar Esquinca, previously un-logged; Pedro confirmed pending / not accepted). Accept rate corrects from 5/11 (45%) to **5/12 (~42%)** — still inside the 40-60% band, still under the 50% working point, still above the 35% floor. Assumption unmoved. Four new sends today (Sep 9) are pending and too fresh to count. Method note: Sasha sends invites by hand in the LinkedIn UI, so the connector's CONNECT quota reads 0 and cannot count them; the reliable counter is get-sent-invitations timestamps.

**2026-09-10 (Day 180) — fresh-cohort accepts running hot, settled rate unmoved.** The Sep-9 batch of 9 now shows **6 accepted within 1-2 days (Diana Chapman, Sophie Vo, Scott Britton, Ron Hill, Ernesto Cacho, Alejandro Morales Heimlich) — ~67%**, well above the 40-60% band. Per this ledger's own rule, a fast early run on a 9-person cohort is signal, not a mover: too fresh (most under 48h) and too small to trust over the settled number. **Settled accept rate holds at 5/12 (~42%).** Reply rate still has n=1 (Danil, Day 177) — three more targets accepted and unmessaged (Toni Mascaró 5 days overdue; the 6 fresh accepts still inside grace) so the reply clock has more denominator waiting than numerator. **Two accepts (Michael Logan Shur, Leonard Khirug) surfaced with no matching sent-invitation record** — untracked sends from outside this session's visible window, not folded into any rate. **Four off-sequence sends found** (Cesar Esquinca, Carlos Quintero — both unaccepted 2nd-degree, messaged anyway; Angel Hernandez, Jeff Cherry — outside the logged Wave 1 target list) — these break Exhibit 4's request-then-message sequence and are excluded from both the accept-rate and reply-rate denominators pending Sasha's call on whether to count them. **Action: forecast held**, nothing here clears the bar to move an assumption.

**2026-09-15 (Day 185) — two more accepts, reply rate still frozen at n=1, forecast held.** Two new accepts since Day 180: Robbie Vitrano (2026-09-11) and Rebecca Carmen Zapf (2026-09-15). Both unmessaged. **Reply rate has not moved in eight days** — `list-conversations` shows zero new activity anywhere in the inbox since 2026-09-10; Danil (Day 177) is still the wave's only reply. The binding constraint is no longer the accept rate (still healthy, 42-78% across cohorts depending on batch) — it is that **nine accepted people are sitting unmessaged**, one as old as 10 days (Toni Mascaró). The reply-rate assumption (20%, 15-25% band) cannot move off n=1 regardless of how many more people accept, because the denominator (first messages actually sent) is not growing. **Action: forecast held** — nothing here clears the sample bar. Flagging for Sasha directly, outside this ledger's scope: the message backlog, not the search or the accept rate, is now the rate-limiting step in the whole funnel.

**2026-09-16 (Day 186) — one more accept, backlog now the binding constraint for a 7th day, forecast held.** One new accept: Nikhil Kale (2026-09-15), unmessaged. Reply rate unmoved — still n=1 (Danil, Day 177); `list-conversations` shows no account-wide activity since 2026-09-10. **Ten accepted people now sitting unmessaged**, Toni Mascaró the oldest at 11 days. Nothing here clears the sample bar on either rate. **Action: forecast held.** Same flag as Day 185, now a day older: the message backlog is the rate-limiting step, not search or accept rate.

**2026-09-20 (Day 190) — one more in-cohort accept, backlog now 11 deep and 3 weeks running, forecast held.** One new accept: Om Chaudhari (2026-09-17), unmessaged — fresh Sep-9 cohort now 8/9 (~89%). Separately, the loose early-wave cohort (generic-founder/crypto/angel, no faculty screen) picked up its first accept — Eugeny Kuznetsov, 2026-09-19 — moving that comparison cohort from 0/7 to 1/7 (~14%), still far below the tight cohort's 42-89% range; reinforces the Day 179 finding that tight targeting outperforms loose. Reply rate unmoved — still n=1 (Danil, Day 177); `list-conversations` shows no account-wide activity since 2026-09-10, now ten days flat. **Eleven accepted people now sitting unmessaged**, Toni Mascaró the oldest at 15 days. Settled 12-cohort accept rate unchanged at 5/12 (~42%); nothing here clears the sample bar on either rate. **Action: forecast held.** Same flag as Day 185-186, now four days older: the message backlog remains the rate-limiting step, not search or accept rate.

**2026-09-21 (Day 191) — no new accepts, Danil's reply re-read as soft defer, forecast held.** No new accepts since Day 190 (`get-connections` recentlyAdded unchanged, still tops out at Eugeny Kuznetsov 2026-09-19). A second, previously-unlogged message from Danil Nikiforov surfaced (sent 2026-09-17): "Спасибо! Интересно вернуться к нашему разговору и послушать твою историю через 1-2 года." This does not add to the reply-rate n (still 1 of 4 people replied) — it is the same person's second message — but it corrects the read of that one reply from "engaged, buying intent" to "curious, then explicitly deferred to a 1-2 year horizon." Reply rate remains unmovable on sample size regardless: the denominator (first messages sent) has not grown since Day 177. **Twelve accepted people now sitting unmessaged**, Toni Mascaró the oldest at 16 days; Om Chaudhari (accepted Sep 17) has now crossed the 2-day grace window into overdue. **Action: forecast held.** The backlog, not the accept rate or the reply rate's sample size, remains the binding constraint, now four days older than the Day 185 flag.

**2026-09-22 (Day 192) — no movement, forecast held.** No new accepts (still tops out at Eugeny Kuznetsov, 2026-09-19) and no new replies (Danil's Sep 17 message remains the newest activity account-wide) since Day 191. Reply rate still n=1 of 4 first-messages sent; unmovable on sample size, not on time elapsed. Twelve people remain accepted and unmessaged, Toni Mascaró now 17 days. **Separately, the matching brief itself moved to v4.31** (`briefs/matchmaker_brief_v4.31.md`) — the primary search territory shifted from the Stream A/B archetype grammar this ledger's cohorts were built on to founder-serving practitioners/operators tested for delivery partnership first. This does not move any rate here; it is a flag that future cohort reads use different grammar than rows 1-30 in the CRM table. **Action: forecast held**, backlog remains the binding constraint.

### How this becomes the game

Every real number is a bet against its assumption. When a rate has enough sample and lands outside its band, it moves, the move is logged here, and the headline forecast (sessions/month, years-to-$1M) recomputes from the moved rate. Green when actual beats assumption, red when it misses, and the $1M clock visibly speeds up or slows down. The one rule that makes it a real game rather than a flattering one: **single data points never move the line — only a stable sample outside the range does.**
