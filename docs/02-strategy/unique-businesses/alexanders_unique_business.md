...
***Monetization Strategy doc added (v1.0).** `docs/02-strategy/monetization_strategy.md` — the new canonical reference for the dynamic, sequenced business-monetization strategy. Captures the formula (PolyForm NC code + 10% rev-share license + forkable kit + opt-in commons = Holonic Franchise), all six plays through Heart/Mind/Gut (Conscious Entrepreneur · Purpose Coach · Platform Distribution · Venture Studio · Community White-Label · System License), tensions + combinations + non-overlap test, four-phase sequenced rollout with explicit exit criteria, implementation status table per play, 16 open decisions tracked. **This document is now the single source of truth** for how the venture earns money across all of its commercial layers, and the place where decisions get tracked as the strategy evolves. Cross-referenced from holonic_franchise_model.md (architectural foundation), unique_business_playbook.md P15 (the brief inline summary now points here for the current state), this file (Day 51 night), and the roadmap. Future versions: v1.1 when first commercial distributor signs · v1.2 when facilitator console ships and first paying coach signs · v2.0 when Phase 2 opens (community white-label) · v3.0 when Phase 3 opens (venture studio) · v4.0 when Phase 4 opens (system license).*
