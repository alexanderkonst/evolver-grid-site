...
> **A new commercial-decentralization pattern: forkable open-source apparatus + per-founder voice generation + opt-in shared commons + 10% tithe to the commons that grew the founder. Distributors become sovereign nodes operating under their own brand, contributing back through a transparent revenue share, and benefiting from a public dataset that compounds in value as the network grows.**
...
3. **The Specificity Loop discovery** (Principle 15) — per-founder matrix generation became technically possible Day 51, which made true sovereignty of distributors operationally real.

Combined, these three forces produced a model that doesn't fit cleanly into existing categories (open-source, franchise, cooperative, platform-as-a-service). Hence the new name: **Holonic Franchise.**
...
[Distributor Agreement](../../DISTRIBUTOR_AGREEMENT.md) v0.1 governs commercial use:
...
The `specificity_matrix` UBB artifact (Day 51, [unique_business_playbook.md](../03-playbooks/unique_business_playbook.md) Principle 15) generates each distributor's voice for every reveal in their funnel. Their funnel speaks in their idiom, not Sasha's. Full identity sovereignty operationally, not just nominally.
...
### Not yet shipped — sequencing decisions for next sprint
- ⏳ **Provisioning automation** — Supabase Management API + Vercel API integration to one-click deploy a distributor instance. Needed for "Skool-easy" onboarding promised in Day 51 conversation.
...
The natural order for the unbuilt pieces:
...
4. **Public "Become a distributor" landing** — only after all of the above is real. Then Sasha can mention the option in any conversation without it being a paper promise.
5. **Commons publication + AI Credits MVP** — parallel after the above. The commons makes the network compound; AI Credits monetize the regenerate-for-higher-resonance loop.

---

## Open Questions for Sasha

The model is structurally complete; these are tuning decisions for v1.1+:

1. **Tier 2 ("Sasha live") definition.** When Sasha is "in the room" for a distributor's clients, the split shifts from 10/90 to 40/60 (per Sasha's Day 51 framing). The exact definition of "in the room" needs to be tightened: 1 full session per client? Recurring quarterly group? Named in the marketing? Otherwise the line will be argued at first invoice.

2. **Pricing autonomy with soft floor.** Distributors set their own session prices. Should there be a soft floor (e.g. "no fire-sale pricing — minimum $X — to protect category-level perception")? Sasha's Day 51 lean: yes, soft floor. Specific number unset.

3. **Brand attribution as soft requirement.** Default Distributor Instance template includes *"Powered by Find Your Top Talent"* in footer. Distributors may remove. Worth tracking how many keep it as a credibility signal vs. how many remove for full white-label.

4. **Anonymization spec for commons.** Drafted as "see separate doc" in [DISTRIBUTOR_AGREEMENT.md §7.3](../../DISTRIBUTOR_AGREEMENT.md). Spec needs writing before commons goes live.

5. **Governing law for Distributor Agreement.** Currently placeholder pending counsel review. Likely Delaware, USA or another neutral common-law jurisdiction. Decide before first signature.

6. **Tier 1.5 brand-borrowing tier?** Discussed Day 51: distributor under their own brand but explicitly references Sasha's methodology in marketing ("Built on the Find Your Top Talent methodology"). Could be a 20/80 tier between platform-only (10%) and Sasha-live (40%). Optional — only matters if brand-borrowing equity proves to be a meaningful asset transfer in practice.
...
*v1.0 · April 25, 2026 (Day 51) · Codified during the Specificity Loop session as the operational model emerging from matrix v2 + license decisions + distributor architecture. Status: shipped at the legal/methodology layer; technical layer (provisioning, Stripe Connect, commons publication) still to build.*
