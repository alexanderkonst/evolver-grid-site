...
- **Inline SVG > CDN PNG for brand marks.** v4 fetched the NS flag from `ns-assets.com` and it rendered as a featureless black rectangle — load issue + aspect mismatch. v5 reconstructs the flag as a 3-rect inline SVG. Ships with the bundle, no third-party request, exact dimensions, paints in the first frame. For any community whose flag/wordmark is a simple geometric mark, prefer inline SVG over external image URL.
