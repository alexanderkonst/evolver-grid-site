...
> **Correction (2026-06-10) — the strategic balance shifted; the FUNNEL default is now match.** During a live demo in front of ~20 people, an attendee on the bare URL walked the build-path reveal ($555/$37 cards) and a locked Mission row — the opposite of the intended flow. Sasha's call: the **match funnel is now the default for everyone**; the legacy sessions funnel requires an explicit `?path=build` link (the link Sasha sends when selling sessions directly). Scope of the flip: post-reveal CTAs, completion CTAs (Mission/Assets/QoL), JOURNEY terminus (#5 Find collaborators + #6 Build a business), celebration-modal suppression, auth Sign-Up default, claim copy. **The landing HERO at `/` is unchanged** — MethodologyLandingPage stays the default; MatchHero still requires explicit `?path=match` (this section's hero table remains accurate). Same day: guest Top Talent completion (on-device `guest_appleseed_data`) now unlocks JOURNEY #2 so the just-revealed guest never sees "Unlocks after you find your top talent." Implementation: predicate convention is `path !== "build"` for funnel flavor, `path === "match"` for the hero swap — see `src/contexts/EntryPathContext.tsx` header.
