...
| **Language switcher** — guest-only chrome + Settings home | ✅ 2026-06-14 (refined) | Was mounted ONLY on the 404 page. First rebuilt as a glassy globe dropdown (English / Русский / Español, native script) at App root `fixed top-right`. Then refined per Sasha (a chip on every page is clutter once the choice persists): `GlobalLanguageSwitcher` now shows ONLY to logged-out visitors (the cold traffic actually choosing) and renders nothing until the session is confirmed absent (no flash); logged-in users change it from a **Settings → Profile "Language"** row. Verified guest sees it on the landing. |
| **System-prompt translation — assessed, NOT translating** | ✅ 2026-06-14 | The Appleseed/ZoG instruction prompts (870 lines, dozens of dated refinements) stay ENGLISH as the single source of truth. Gemini follows English meta-instructions and outputs in the target language when directed; translating would triple maintenance + risk drift + fork the source. Output language + register handled by `languageDirective` + `revealCalibration` (already shipped). Verified no prompt fragment injects verbatim English into the artifact. |
| **Debug audit** — 41-agent adversarial sweep, 25 confirmed findings | ✅ 2026-06-14 | Fixed all 5 HIGH + the correctness-MEDIUM. **Plurals:** `statSteps` was leaking a raw key into the ENGLISH default UI (RU one/few/many authored in en+es, no `_other`) — fixed; RU few/many added for holonicModules; full RU/ES plural sets for asset/streak/word keys (CLDR-verified via `Intl.PluralRules`). **Redirects:** new `localizedOrigin()` helper — magic-link/OAuth/Stripe/share URLs no longer drop the /ru,/es prefix. **Layer 3:** `generate-blueprint` + deepen-icp/pain/tp + suggest-next-quest + match-assets now thread `target_language` (were silent-English). **Formatting:** dates → `formatDate`, XP numbers → `formatNumber`, event time → new `formatTime` (24h for RU/ES), dashboard → compact currency. |
| **Debug audit — 2nd pass** (18-agent adversarial), 13 findings | ✅ 2026-06-14 | No regressions from pass 1 (localizedOrigin, guest-gating, edge fns, plurals, new keys all verified). Surfaced + fixed the logged-in GAME SHELL components extraction had MISSED: SpacesRail's 8 nav labels (English on every authed page), the mobile breadcrumb + pane-2 group title (now "CAMINO" not "JOURNEY" — shared `spacesRail.*` keys across rail/breadcrumb/pane), the daily-loop cluster (NextMoveCard + starter-action titles, MeSummary, MeSection, EmptyStateCard), the COLLABORATE rail, and four whole pages (ProfileOverview, MyProductsPage, EventsSpace, AppleseedView). +146 keys via a one-agent-per-component extraction wave, merged centrally, all wired (verified, no lost edits), tsc + production build green. Also dropped the banned "Genius Business" brand from the /ignite logo alt. |
| 2q+ — remaining LOW debug items (deferred) | ⏳ | `preferred_language` types.ts cast debt (latent, works); raw `window.location.href` SPA-reload prefix drop (useStepCheckout free-step, signout, Index, ToolsRedirect — localStorage usually retains); use-stripe-portal.ts error toasts (hook, reachable from Settings — needs `t` injection); suggest-asset-matches + match-missions language; chart.tsx tooltip number grouping; ProfileMissionSection empty-state straggler; generate-qol-guidance dead path (info). |
| 2q+ — landmines (15 transactional emails — parked per onboarding pathway, `preferred_language` infra ready) · holomap/admin (deferred) | ⏳ | parked / deferred |

**Running tally (2026-06-14): EN catalog ~4,500 keys; RU + ES at full parity. ~185 files, 20+ production builds green; live-verified on a fresh dev server (RU + ES render, guest switcher in place, "CAMINO" not "JOURNEY", zero console errors). The full UI + 5 standalone data hubs + the logged-in game shell + Layer-3 generation (with reveal-specific RU/ES calibration) read in BOTH Russian and Spanish; a guest-only language switcher + Settings row make RU/ES discoverable without cluttering the deep app; SEO hreflang/og:locale in place; TWO adversarial debug audits ran (41 + 18 agents) and all HIGH + correctness-MEDIUM findings are fixed, no regressions.** What's left is all either explicitly deferred, parked, or low-priority polish, none of it on the first-impression path: the mission-discovery wizard (gated on the 583-mission catalog Sasha said skip — one word unlocks it), 15 transactional emails (parked per onboarding pathway; `preferred_language` infra ready), seeded Supabase quotes (stay in original voice — real-person content), admin surfaces (deferred), an SSR/edge layer for first-paint SEO (Phase 3), and the LOW debug stragglers listed above. Phase 0b: calendar locale + preferred_language sync + format sweep + output_language done; Zod errorMap minimal (1 schema) deferred.
