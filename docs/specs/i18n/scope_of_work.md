...
| 2q+ — remaining data hubs (missions catalog ~2,300 strings — **reference, skipped unless asked**) · landmines (15 emails/charts/PDF) · holomap/admin (deferred) | ⏳ | reference-heavy bulk |

**Running tally (2026-06-14): EN catalog 4,355 keys; RU + ES at full parity (4,355 each). ~146 files, 16 production builds green. The full UI + 5 standalone data hubs + Layer-3 generation (with reveal-specific RU/ES calibration) read in BOTH Russian and Spanish; SEO hreflang/og:locale in place.** Remaining is reference/transactional content: missions seed catalog (~2,300 — reference, skipped per Sasha unless asked), 15 transactional emails, admin surfaces (deferred), and an SSR/edge layer for first-paint SEO. Seeded Supabase quotes deliberately stay in original voice (real-person content). Phase 0b: calendar locale + preferred_language sync + format sweep + output_language done; Zod errorMap minimal (1 schema) deferred.
