...
| **Language switcher** — global, discoverable | ✅ 2026-06-14 | Was mounted ONLY on the 404 page (RU/ES unreachable by organic traffic). Rebuilt as a polished glassy globe dropdown (English / Русский / Español, native script) mounted once at App root `fixed top-right` — lands in the empty funnel-header corner, visible on the funnel landing + in-app shell + marketing. Preserves path+query+hash+skin via `buildLocalePath`, persists to localStorage. Verified live. |
