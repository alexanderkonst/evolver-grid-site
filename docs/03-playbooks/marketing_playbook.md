...
- Logical arguments about product quality
...
## Attraction-Mode Marketing — When the Founder IS the Funnel
...
This section covers a marketing mode that does NOT use ads, paid acquisition funnels, or split-tested message-market-fit optimization. It applies when the founder's business model is structurally inseparable from them (Founder-as-Body-of-Work pattern — see UBP §16). For traditional product marketing, this section does not apply; use the Direct-Response sections above.

### The structural difference from persuasion-mode

Standard funnel: ad → landing → email → drip → trial → paid. Each step is friction-reducing machinery designed to move a stranger through psychological resistance. Cost per acquisition scales linearly with reach.

Attraction funnel: artifact → use → recognition → repeat use → mention to others → seek the source. The artifact carries all the weight. The funnel collapses into the artifact itself. Cost per acquisition is near-zero at the margin (each artifact use carries the founder's attribution into others' contexts at no marginal cost).

**Why this is structural for Founder-as-Body-of-Work businesses:** when the business model IS the founder's identity, there is no separable "product" to advertise. The ad would have to be "be near me" — which doesn't compute as a paid placement. You can only **be who you are, in a way that's discoverable.**

### The 10 layers of attraction-mode (depth, in order)

1. **Resonance vs. persuasion.** The work either lands or it doesn't. No friction-overcoming machinery.
2. **Cost asymmetry.** Each artifact use is free distribution carrying attribution; ads cost per click.
3. **Time horizons.** Encounter-to-deep-engagement is months/years, not days. Compounding curve, not linear ROI.
4. **Selection-by-resonance, not by targeting.** The work filters the audience; you don't have to.
5. **Funnel collapses into the artifact.** No middleware between encounter and offer.
6. **Identity coherence vs. message-market fit.** You double down on what's most true rather than testing what converts.
7. **The asymmetric advantage.** Being yourself visibly + consistently is the highest-leverage marketing for this business model. No ad copy you could write would be more attractive than the actual work in public.
8. **The trap is impatience.** When attraction grows slower than your impatience, the temptation is to add ads. Ads dilute the aligned tribe — once mixed, hard to un-mix.
9. **The other shadow is passivity.** "I'll just be myself and the right people will come" — wrong. Attraction requires *visible work, in public, consistently, where the aligned tribe gathers*. Active verb is **broadcast**, not **target**.
10. **The work IS the broadcast.** Every public conversation, every artifact, every install is the broadcast. The hook IS the delivery; the methodology IS the offer; the founder IS the broadcast medium.
...
- **Discipline:** if you need to push it, it's not ready (per `planetary_os_assembly.md` Principle 5). Refine until aligned tribe shows up unprompted.

### What to NOT do

- ❌ Run paid acquisition funnels for the founder-as-body-of-work tier
- ❌ Generic / mass-targeting outreach
- ❌ Manufactured urgency / scarcity in copy (real scarcity = founder's time = let it be visible, don't fake it)
- ❌ A/B test your authentic frequency to find what converts (you'll lose the signal)
- ❌ Lead magnets that gate the methodology — the methodology IS the magnet

### When to revisit and add ads

If the business outgrows founder-bandwidth (i.e., revenue is bottlenecked by your hours and you've productized the lower tiers into trained-collaborator delivery), you may revisit ads for the LOWER tiers (free scaffold awareness, mass distribution of the methodology) — never for the upper tiers (sessions, advisory). The upper tiers stay attraction-only because they sell proximity-to-source, which cannot be manufactured by ads.

---

*Updated: April 28, 2026 (Day 54+) — Attraction-Mode Marketing section added to operationalize UBP §16 (The Self-Monetizing Founder). Captures the 10-layer attraction-mode taxonomy + operational guidance + anti-patterns for Founder-as-Body-of-Work businesses. Cross-references `communications_playbook.md` (DM template), `distribution_playbook.md` (segment + channel detail), `unique_business_playbook.md` §16 (the principle).*
