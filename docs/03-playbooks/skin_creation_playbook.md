...
- **Phase Shift Technology 87 — Multi-Patron Architecture:** if a new skin costs 30-60 min, the org can sustain N patrons in parallel. If a new skin costs 30-60 days, the model collapses to "find one big patron and pray." Cheap skins are what makes Leonardo-strategy multi-patron commerce tractable.
- **Phase Shift Technology 88 — Demo-as-Hospitality (The Skinned Welcome):** the conversational opening with an ecosystem leader is not the pitch, it is the URL. You build the counterparty's skin BEFORE the conversation, then open with *"here's what your version already looks like."* This move flips pitch-energy to hospitality-energy. It only works when the skin is cheap enough to build speculatively, which this playbook is what makes possible.
...
| **Outer `bg-[#0a0a1a]` GameShell wrapper smothers body bg image OR gradient** | Solid color fill on the outer wrapper covers ANY body-level bg content (image, gradient, radial). On desktop masked by opaque pane backgrounds; on mobile exposed as flat gray/sage wash. Recurring trap — first hit on planetir bg image (Day 84), again on darktheme bg gradient (Day 88) | Override to `background-color: transparent !important` (not a color) regardless of whether body uses image or gradient. The wrapper must NEVER paint solid color under any non-Aurora skin |
...
- **Outer wrapper `bg-[#0a0a1a]` cascade RE-STRIKE on darktheme.** Day 84 v2 cross-skin lesson said this wrapper must be `transparent` when the body uses a bg-image. The first darktheme pass overlooked it — I painted the wrapper SOLID `#08080a` reasoning that no bg-image was present so the body bg gradient could be smothered safely. WRONG. The body bg GRADIENT is also chromatic content that the wrapper smothers when painted solid. On desktop this was masked by opaque pane backgrounds covering the wrapper; on mobile (no opaque pane to cover) the wrapper's flat color was visible as a gray/sage wash, with the body's amber radial completely hidden. **Fix:** `[data-skin="<slug>"] .bg-\[\#0a0a1a\] { background-color: transparent !important }` regardless of whether body uses a bg-image or a bg-gradient. The wrapper must NEVER paint solid color under any custom-bg skin. **Promoted to cross-skin pattern catalog as a stronger rule:** "Outer wrapper `bg-[#0a0a1a]` → transparent under any non-Aurora skin with body-level bg content. No exceptions. The wrapper smothers gradients and images equally."

- **Deepening pass methodology.** When a skin's base luminance needs to shift after the first pass (e.g., user wants it closer to true black), the change lives in ONE coordinated block: (1) the token CSS vars `--skin-panel-1-bg`, `--skin-panel-2-bg`, `--skin-panel-wash`; (2) the body bg gradient stops; (3) the `.liquid-glass` / `.liquid-glass-strong` bg + box-shadow rgba values; (4) the shadcn HSL `--background` / `--card` / `--popover` luminance. All shift together by the same delta (~3-6% L in HSL) to keep the relative gradient hierarchy intact. **Lesson:** never deepen the body bg without ALSO deepening the glass elements — otherwise the panes start floating brighter than the canvas. Coordinated luminance shift, not isolated.
...
### Techstars pass — sixth distinct CTA dialect, first non-warm-accent skin (2026-06-08, Day 91)

**Built in ~25 min build + QA wall-clock, zero correction rounds, zero v1→v2 iteration**. Tying Darktheme for the playbook floor. Validates that the spec generalizes to high-recognition global brands (Techstars vs. boutique community demos) without slowing throughput.

- **6th distinct CTA dialect — solid bright Techstars green.** First non-Aurora CTA whose accent hue is OUTSIDE the warm-amber / gold / yellow family that Aurora, Daouniverse, Karime, and Darktheme all converge on. Pure `#3DCC4A` with black text. **Lesson:** the cross-skin pattern catalog had previously assumed all primary CTAs would land somewhere in the warm-amber neighborhood; the `bg-[#d4af37]` re-tone rule (Day 88 v3) needed a non-amber target color for the first time. Confirmed the re-tone rules ARE color-agnostic — the target color is just a token swap.
- **Free-font near-match strategy for paid display fonts.** Techstars uses Suisse Intl (paid Swiss Typefaces face, no Google Fonts equivalent). Inter (already loaded for NS/Daouniverse/Planetir/Karime) is the closest free Swiss neo-grotesque near-match — same register, broadly compatible weights + glyphs. **Lesson:** when the source site uses a paid display font, default to the closest free near-match in the platform's existing font load rather than buying a license or shipping a 3rd-party CDN font. Note the substitution explicitly in inventory ("Inter near-match for paid Suisse Intl") so future-you doesn't claim verbatim parity.
- **Video filter recipe IS part of the inventory.** Techstars darkens their hero video heavily (`brightness(0.62) saturate(0.78) contrast(1.04)`) so the headline is legible on a busy crowd-photo. **Lesson:** for photographic Mux bgs, eyedrop the source site's filter recipe from their actual video element via the Next.js source-CSS extraction technique and apply it as a `[data-skin="X"] video[autoplay]` rule. Don't try to compensate for a too-bright video with heavier text halos; the source brand already solved the problem with a filter.
- **Heavy-photo bg + bold display headlines = halo budget MUST be deeper.** The Mux video is a sharp full-color photo (Startup Weekend group, balloons, papers). `--skin-text-halo-deep` needed `rgba(0,0,0,0.85)` at 36px blur (vs darktheme's 28px at 0.72). **Lesson:** photo bgs need ~30% more halo than abstract-gradient bgs for the same legibility. Inventory step should note "photographic vs abstract bg" and scale halo accordingly.
- **The 4-pattern green re-tone catalog.** Aurora yellow `rgba(244,212,114,…)`, Aurora gold `rgba(212,175,55,…)`, Tailwind arbitrary `bg-[#d4af37]`, AND the `bg-[#d4af37]` ::shadow combo — all four needed scoped re-tones to `#3DCC4A` family. Confirmed the same 4-pattern catalog applies cleanly when the target hue shifts 90°. Future skin in any hue can fork this template directly.
- **`isFullRailScope` is now load-bearing for FIVE skin scopes.** Planetir + Daouniverse + NS + Darktheme + Techstars all need the full rail visible to unauthed visitors. The `__isFullRailScope` flag (renamed from `__isWhiteLabelDemoScope` because darktheme broke the white-label semantic) is the canonical decision point. **Lesson:** keep this flag's name semantic-of-purpose ("full rail visible to guests"), not semantic-of-implementation ("demo scope"), so each new skin gets a clear yes/no decision.

### Throughput claim (validated at N=5 skins + Planetir 2 fix rounds)
...
| **Techstars** | **~25 min build + QA, zero correction rounds** | **0** | **Full SoW+DoD discipline; first non-warm-accent skin (#3DCC4A green); Inter near-match for paid Suisse Intl** |

**Direction of improvement is monotonic.** Each new skin ships faster than the prior because the spec absorbs the prior's lessons. Techstars sets the new floor at ~25 min build+QA and proves the playbook generalizes across hue families (green vs the warm-amber neighborhood that Aurora/Daouniverse/Karime/Darktheme share).

### Day 91 — Lapis/Aurum platform-toggle pass (2026-06-09)

**The dark skin graduated from a route-scoped preview to a first-class user-selectable theme.** Slugs renamed (`aurora`→`lapis`, `darktheme`→`aurum`) with localStorage migration; toggle shipped in the rail + Settings → Appearance; 159 light-leak findings remediated across two Workflow fan-outs (8 agents each). New permanent lessons:

- **Route scope ≠ skin value.** Once a skin is persistable, every behavioral gate keyed on the skin VALUE silently extends to users who merely prefer it. The full-guest-rail gate keyed on `skin === "darktheme"` would have shown the demo rail to every dark-preferring guest on the canonical funnel. Demo-only behavior must key on route scope (`src/lib/skinScope.ts` `initialSkinScope`), never on skin value. Same class of bug in PreviewBanner (permanent chip + Exit clobbering the choice), `/preview` (persisting navy-gold over the user's theme), MuxVideoBackground (streaming hidden video), chart palettes and dark-hero branches checking only `navy-gold`.
- **Const-indirection hides literals from every sweep.** `const INK = "#0a1628"` applied via JSX is invisible to class-based CSS re-tones AND to source greps for style-attr literals (the recon missed ProfileMissionSection entirely; Sasha caught it on device). When auditing, also grep for `= "#` const assignments.
- **Greedy attribute-substring rules nuke border-only matches.** `[style*="rgba(212, 175, 55"]` was written for gold FILLS but also matched elements whose only gold was an inline BORDER, and (coming later at equal specificity) its `background-color !important` overrode the dark card re-tone — the Mission card rendered as a washed amber-cream slab. Re-tone rules that set fills must require `[style*="background"]` in the selector.
- **Token-resolution fidelity: undefined-at-root alias tokens.** Tokenizing `rgba(255,255,255,0.85)` as `var(--skin-card-bg, …)` looks safe, but `:root` defines `--skin-card-bg: …0.65` — the token RESOLVES and the fallback never fires, so the light skin drifts. For exact-fidelity remediation use alias tokens defined ONLY in the dark blocks (`--skin-card-fill`, `--skin-input-fill`, `--skin-ink`, `--skin-ink-body`, `--skin-ink-muted`, `--skin-page-bg`, `--skin-page-wash`): dark skins resolve them; the light skin always falls back to the exact original literal.
- **Pre-paint script is mandatory for a persistable dark theme.** Without an inline `<head>` script applying `data-skin` from localStorage (with the legacy-slug migration map inlined), dark users get a light flash on every load. Keep the inline map in sync with `LEGACY_SKIN_MIGRATIONS` in SkinContext.
- **Restore, don't reset.** Any "exit preview" affordance must restore the user's persisted theme (`getPersistedSkin()`), not hard-set the default.

---

*Skin Creation Playbook v1.4*
*v1.0 created 2026-05-25 (Day 84 evening) — extracted from `white_label_strategy.md` v1.10. · v1.1 (Day 84) — Planetir round-2 fix-pass: 9 lessons across 4 sections. · v1.2 (2026-05-30 Day 88) — Darktheme pass: first non-white-label skin authored from scratch. CTA dialect catalog expanded to 5 paradigms (amber-glass pill added). · v1.3 (2026-06-08 Day 91) — Techstars pass: 6th CTA dialect (solid green, first non-warm-accent), free-font near-match strategy, video filter recipe in inventory, halo budget for photo bgs, 4-pattern Aurora-gold re-tone catalog confirmed color-agnostic, `__isFullRailScope` now load-bearing for 5 skin scopes. Throughput floor lowered to ~25 min build+QA. · v1.4 (2026-06-09 Day 91) — Lapis/Aurum platform-toggle pass: dark skin graduated to a first-class user toggle; route-scope vs skin-value decoupling doctrine; const-indirection + greedy-substring-rule + token-resolution-fidelity lessons; exact-fidelity alias-token method for leak remediation at scale.*