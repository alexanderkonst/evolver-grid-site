...
| **"ship to main"** | Take this worktree's net-new work and land it on local main without drift surprises. **Steps:** (1) audit — list committed branch commits + uncommitted files; (2) diff vs `origin/main` — identify net-new vs already-shipped (skip dupes); (3) pick path automatically — clean branch commits → push branch + open PR; uncommitted-only → copy files to main checkout + commit there; mixed → both in order; (4) build + typecheck on main BEFORE committing to catch regressions; (5) commit on local main with focused messages; (6) **never push** unless the `and push` variant is used; (7) report worktree-cleanup one-liner at the end. **Variants:** `ship to main and push` (also `git push origin main`), `ship to main as PR` (always PR, even for tiny stuff), `ship to main and clean up` (also `git worktree remove --force` the source worktree at the end), `ship to main, dry run` (show the diff plan, apply nothing). **Why this verb exists:** Claude Code's auto-worktree pattern silently bases new sessions on whatever main was at session-start; if main moves forward during the session, the worktree drifts. `claude-sync` (shell function in `~/.zshrc`) pulls main into the worktree at session-start; `ship to main` lands the worktree's work the other direction. Pair them and drift can't bite. | Local main commit + worktree files |
