...
| `has_ignition` | bool | EXISTS Stripe checkout with `amount = 555_00` AND `status = 'paid'` |
| `has_build` | bool | EXISTS Stripe checkout with `amount ≥ 1111_00` AND metadata tag `container=build` |
| `revenue_total_usd` | numeric | `SUM(stripe.charges.amount_paid) / 100` |
| `last_touch_at` | timestamptz | latest of `onboarding_stage_updated_at`, any snapshot insert, any Stripe event |
...
- Card: Container flags — Ignition ✓ / Build ✓ with purchase timestamps
- Card: Revenue timeline — list of Stripe charges
...
- Revenue total + MRR estimate
- New founders in last 7 days
- Founders who've completed Ignition but not booked Build (action surface)
...
4. **Stripe amount thresholds are placeholders.** Confirm the actual Stripe product IDs / metadata pattern before hardcoding `amount = 555_00`. Ideally key off a `stripe_products` table or Stripe metadata (`container: 'ignition'` / `container: 'build'`).
...
- [ ] `/admin/dashboard` renders aggregates
