...
-- Per-user storage of the 7-artifact unique-business canvas.
-- Powers the JOURNEY space "My Artifacts" view (Sasha, 2026-04-21).
--
-- One row per (user_id, artifact_key, version). A single user can keep
-- versioned copies of the same artifact as precision improves (v1 → v1.1 → v2…).
-- RLS: read/write own rows only. Public portfolio flag comes later.
...
-- Index for the common query: "my artifacts, grouped by step."
...
-- Row Level Security — users can only see/edit their own artifacts.
...
-- Seed: Sasha's canvas snapshot (v9.4). Idempotent — safe to re-run.
-- Looks up his user_id by email; silently skips if the auth row doesn't exist yet.
do $$
declare
  sasha_id uuid;
begin
