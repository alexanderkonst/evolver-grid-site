...
function buildBriefPrompt({ brief_path, briefContent, mode }) {
  ...
  return [
    ...
    ``,
    `When you are done:`,
    `  1. Run \`npm run test\` and \`npm run corpus:drift\` — both must be green.`,
    `  2. Rename the brief from ${brief_path} to its DONE_*.md equivalent (swap PENDING_ for DONE_).`,
    `  3. At the bottom of the DONE file, add a "Notes from execution" section covering: what changed vs the brief, any pattern divergences, and the names of new files/migrations.`,
  ].join("\n");
}
