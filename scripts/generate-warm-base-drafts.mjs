  ...
  return [...records.values()]
    ...
    .map((record) => {
      const hasPlatformTalent =
        record.talentRevealCompleted === "yes" && Boolean(record.topTalent);
      const language =
        record.source === "platform" && !hasPlatformTalent
          ? "en"
          : isRussian(record)
            ? "ru"
            : "en";
...
function createWarmBaseDrafts() {
  ...
  WARM_BASE_DRAFTS.forEach(function (draft) {
    ...
    // Skip anyone already emailed (the batch sent before this regeneration).
    const alreadySent = GmailApp.search("in:sent to:" + draft.email + " newer_than:7d");
    if (alreadySent.length > 0) {
      properties.setProperty(key, "already-sent");
      skipped += 1;
      return;
    }
